...
```
   ...
   │        ▼                                                                │
   │  decide()                                                     verdict.ts │
   │        ≤ 84 suspicious  ·  85 to 192 unknown  ·  ≥ 193 trusted          │
   │        │                                                                │
   │        ▼                                                                │
...
EOAs that sent USDC on Base in blocks 50021246 to 50021255) → `scripts/collect.ts` →
...
| Blockscout answers 500 | 1 retry (~0.5 to 0.75 s backoff), then the **server** replays a committed capture, or a cache entry of any age, if it has either (`X-KYA-Degraded: upstream`), else `502` carrying the real upstream message. The **gate** does not degrade: `503` | No |
