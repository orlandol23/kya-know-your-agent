...
> commit-by-commit plan that accompanies [`SECURITY-FIX-BRIEFING.md`](SECURITY-FIX-BRIEFING.md),
...
Caminho até a rede: `gate.ts:99` → `gate.ts:107` → `verify.ts:80` → `history.ts:164`
...
- **Sem travessão.** `README.md`, `DECISIONS.md` e os três arquivos de `docs/`
  têm **zero** caracteres `—` e `–` (conferido: commit `b570bf4`, *"docs: replace
  ...
  ```
- **Títulos bilíngues.** `DECISIONS.md` e `PLAN.md` usam `## English (Português)`
...
- **Comentário explica POR QUÊ, não o quê.** Todo docstring do repo justifica uma
  decisão e cita o número medido. Mantenha o registro ao reescrever.

## 1.2 Ambiente e ferramentas

- Node >= 22. `npm test` é `tsx --test`, que descobre `test/kya.test.ts` sozinho.
...
- **`test/kya.test.ts:23-25` seta `ATTESTER_PRIVATE_KEY` no topo do módulo**, com
  o comentário *"Set before any verify() runs: attesterAccount() reads the env at
  call time"*. O teste #6 mexe em variáveis de ambiente: se mexer antes dessa
  linha, ou não restaurar, **quebra os testes 1 a 5**. Restaure tudo no `finally`.
- **`isOffline()` lê `process.env.KYA_OFFLINE` na hora da chamada**
  (`history.ts:79`) e `setOffline()` muta `process.env` (`:83-86`). Não há estado
...
typecheck sem nenhuma saída (exit 0), e `git status --short` mostrando **apenas**
...
resto do código já usa (`apiUrl()` em `blockscout.ts:155`, `chainId()` em `:163`,
...
`# pass 5`, `# fail 0`, typecheck limpo, e o grep **sem nenhuma saída**. Qualquer
...
**Arquivo:** `test/kya.test.ts`, no fim, seguindo a numeração e o estilo dos cinco
existentes.
...
- **Contagem por stub do `globalThis.fetch`**, deixando passar tudo que não for o
  host do Blockscout (o próprio teste usa `fetch` contra `127.0.0.1`). Restaurar
  o `fetch` original no `finally`.
- **`paymentMiddleware` REAL do `x402-express`**, apontado para um facilitator do
...
na asserção de controle), **o teste está errado, não o código.** Conserte o teste
...
usam palavra direcional que inverte (`ARCHITECTURE.md:25` diz *"mounts **in front
...
o gate roda antes é ponto que escapou. Rode antes de fechar o commit, não depois.
...
>
> O gate deixou de estar "na frente do x402" e passou a estar **dentro dele**, na
...
fixture para o endereço derivado dessa chave, que não existe, e responde **503 em
...
4. **`.env.example`**: `DEMO_ESTABLISHED_PRIVATE_KEY` deixa de ser "`--real` only"
...
Tem que terminar com o fresh em **403**, não 503. Um 503 significa que o gate não
...
1. Extrair `server.ts:95-140` para `src/budget.ts`, sem mudança de comportamento.
...
gate em offline procura `data/fixtures/<endereço derivado>.json`, que não existe,
...
5. O `0xeB94Dd…6B97` **continua no repositório**: é um dos quatro casos do teste
   #2 (`test/kya.test.ts:72`, score 0). Não remover, só deixar de ser o agente da
   demo.

**Ponto de verificação:** é o 4.3. `npm run demo:agents:offline` tem que terminar
com o fresh em **403**, e `npm test` tem que continuar com os quatro fixtures do
teste #2 intactos.

**Custo:** ~1 hora, mais 6 créditos de Blockscout.

**O contra, registrado:** uma chave privada commitada num repositório cujo assunto
agora é uma falha de segurança. É por isso que o comentário do passo 3 não é
opcional: um revisor apressado vê "private key" no diff e tira a conclusão errada.
...
permissivo, e imprimir na tela um aviso de que aquele modo não valida assinatura.
...
**Foi rejeitada mesmo assim, e o motivo é o próprio custo dela.** Exigir um aviso
