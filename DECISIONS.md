...
escala custodial, não identidade. Agora existe fonte. O modelo
`cex_evms.addresses`
...
`scripts/build-cex-labels.ts` reconstrói o arquivo byte a byte a partir do
commit
pinado. Reprodutível, não confiado na minha palavra.

O `lookupCex` roda ANTES da heurística e devolve identity `confirmed`; o miss
cai
na heurística de sempre, com identity `inferred`. A CLASSE é `exchange` nos dois
caminhos. Isso é deliberado: o score lê `class` e nada mais, então a fonte
melhora
...
O substituto é `0xeA258496a9311Ffe29CDf920Ca0E8BB4B41c9F04`: sem tags, risk
score
...
O `0x2CfF` PERMANECE em `data/addresses.csv` e `data/signals.csv`. A calibração
é
...
limite real da v0.1, não um detalhe de apresentação, e a lista de sanções do
OFAC
...
Sem fonte que responda, o eixo só poderia ser preenchido com a diversidade que
já
...
**O Reputation Registry do ERC-8004, medido como implantado, não está
saudável.**
...
tela, sem API. O que o KYA lê é o histórico GERAL da Base, por uma API pública,
e
...
Como o erro passou: a checagem do D18 confirmou o que o x402scan É (explorador
da
...
A v0.1 subiu numa URL pública porque a banca pediu. Uma URL pública muda o
modelo
...
do dia UTC, cap em KYA_DAILY_VERIFY_BUDGET (default 500). Só leitura que chega
na
Blockscout é cobrada: cache hit sai de graça, que é exatamente o tráfego dos
endereços da demo sendo clicados. É orçamento, não cerca: requisições
concorrentes
podem passar pela checagem antes de qualquer uma cobrar, e o excesso fica
limitado
...
avaliação que responde 502 sob abuso não serve para nada, e a alternativa
honesta
já existia: fixture é captura datada de endereço vivo, não mock. Sem fixture
para
o endereço, os dois motivos se separam. Orçamento estourado é este serviço
limitando o chamador: 429, com Retry-After até 00:00 UTC. Blockscout fora do ar
é
falha de upstream: 502, com a mensagem real do upstream, não com a do fixture
que
...
nada: responder com uma captura datada, rotulada como tal, é mais útil que um
502
...
publicado no README, fora de banda, porque "verificável sem confiar nesta API"
só
significa alguma coisa se a API não for quem diz em quem confiar. A consequência
é que uma resposta ao vivo traz um attester diferente do dos exemplos, e o
README
avisa disso em vez de deixar quem comparar achar que encontrou uma
inconsistência.
...
(a atualização de dependência vira PR separado, validado com lockfile, testes
e typecheck); itens 5 e 6 com os itens 13 e 14, os quatro a mesma superfície
de execução: configuração na montagem, testes dos caminhos críticos, lint no
CI e o teste de ponta a ponta do x402; item 7; item 8;
...
Segunda passada, 07/09/2026, além da auditoria (itens 9 a 12, também
congelados):
...
15. **High de dependência: axios e ws, os dois transitivos pelo
    `x402-express`.** Re-auditado em 10/09/2026, commit `36b1ee7`, com
    `npm audit --omit=dev`: 32 entradas, 2 high e 30 moderate; a árvore
    completa com dev devolve a mesma contagem, porque o dev (`@types/*`,
    `typescript`) não contribui e tudo que foi achado é dependência de
    runtime. Os 2 high são pacotes distintos, e os dois transitivos: o
    registro de 09/09 abaixo confundia os dois num caminho só. **axios**
    1.16.0, 10 advisories (1 high, `GHSA-gcfj-64vw-6mp9`), via
    `@coinbase/cdp-sdk`; **ws** 8.18.0, 2 advisories (1 high,
    `GHSA-96hv-2xvq-fx4p`), via `x402` → wagmi → `@walletconnect` →
    `viem@2.23.2`. Nenhum é dependência direta e nada em `src/` importa
    nenhum dos dois. A promessa de um `npm audit fix` "simples, sem
    quebrar nada" **está retirada**: o `--dry-run` e a aplicação do fix
    numa cópia descartável da árvore mostram que ele só sobe
    `body-parser`, `qs`, `hono` e `@metamask/utils` (32 → 30) e não toca
    em axios nem em ws. Atualizar axios é trabalho separado — PR próprio,
    validado com lockfile, suíte offline e typecheck. ws só sai com o
    downgrade quebrante do `x402-express` a 0.4.1, que continua não sendo
    feito; os moderates ficam para o Dependabot do item 12. Este item não
    afirma os high resolvidos; o lockfile não mudou. O registro original,
    mantido aqui como o que foi dito na época: `npm audit --omit=dev`
    reexecutado em 09/09 registrou "axios 1.0.0 a 1.17.0 em 2 advisories
    high, alcançados pelo `@coinbase/cdp-sdk`, dependência do
    `x402-express`", com fix não quebrante — não reproduzido e corrigido
    acima e em `docs/AUDIT-2026-09.md`.
